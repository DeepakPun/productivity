    ...
    <nav className='fixed top-0 left-0 right-0 h-14 bg-slate-950/80 backdrop-blur border-b border-slate-800 px-6 flex items-center justify-between z-50'>
      ...
      <div className='flex items-center gap-4 relative' ref={dropdownRef}>
        <div className='hidden sm:flex items-center gap-2 text-xs font-mono text-slate-400 bg-slate-900 border border-slate-800 px-2.5 py-1 rounded'>
          <span
            className={`h-1.5 w-1.5 rounded-full mr-1 ${isAuthenticated ? 'bg-emerald-400 animate-pulse' : 'bg-rose-500'}`}
          ></span>
          {isAuthenticated ? 'System Core Active' : 'Access Restricted'}
        </div>
        ...
        {isAuthenticated && (
          <>
            ...
            >
              <span>
                {currentActiveItem ? currentActiveItem.label : 'Menu'}
              </span>
              <span
                className={`transition-transform duration-200 block text-[9px] ${isOpen ? 'rotate-180' : ''}`}
              >
                ▼
              </span>
            </button>
                  ...
                  {MENU_ITEMS.map(item => {
                    ...
                    return (
                      <Link
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2 text-xs font-medium rounded-lg text-left border transition-all ${isActive ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40' : 'bg-transparent text-slate-400 border-transparent hover:bg-slate-900 hover:text-slate-200'}`}
                      >
                        <span className='text-sm'>{item.icon}</span>
                        {item.label}
                      </Link>
